...
# ============================================================================
# Clover Compiler Builder v14.1 - Enhanced Version
...
# Instalar setuptools automaticamente
install_setuptools() {
    local python_cmd="$1"
    ...
    # Instalar setuptools
    info "Instalando setuptools via pip..."
    if "$python_cmd" -m pip install setuptools 2>/dev/null; then
        ok "setuptools instalado com sucesso!"
        ...
        err "Falha ao instalar setuptools via pip"
        return 1
    fi
}
...
fix_distutils_issue() {
    ...
    if check_distutils "$python_cmd"; then
        ok "distutils.util está funcionando corretamente"
        return 0
    fi
...
configure_python() {
    ...
    # Primeiro, verificar se Anaconda está disponível
    if check_anaconda; then
        anaconda_available=true
        info "Anaconda detectado - será priorizado"
    else
    ...
    # Se não encontrou Python compatível, instalar
    if [ "$found" = false ]; then
        warn "Nenhum Python compatível encontrado!"
        echo
        info "Recomendações:"
        info "1) Instalar Anaconda 3.9.x (melhor opção)"
        info "2) Instalar Python via Homebrew"
        echo
        
        read -p "Deseja instalar Python 3.9 via Homebrew? (s/N): " answer
        case "$answer" in
            [Ss]*)
                if ! command -v brew >/dev/null 2>&1; then
                    install_homebrew
                ...
                PYTHON_VERSION=$(python3.9 --version 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | cut -d' ' -f2)
                PYTHON_PATH=$(which python3.9)
                found=true
                ;;
            *)
                err "Python compatível é obrigatório"
                exit 1
                ;;
        esac
    fi
...
generate_report() {
    ...
    # Check distutils status
    if check_distutils "$PYTHON_CMD" >/dev/null 2>&1; then
        ok "distutils.util: ✅ Funcionando"
    else
